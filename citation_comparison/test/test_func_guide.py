...
#The test script will have 6 functions, each testing different \
#functionality of the read_PWCjson function. When testing, \
#6 tests will be performed and if all goes according to plan, \
#testing should result in 4 tests passed.

@pytest.mark.load_missing
def test_file_load_missing_url_file():
    '''Test if the function raises the exception \
    if nonexisting URL_file name is passed as parameter.'''
    with pytest.raises(ValueError) as excinfo:
        read_pwcjson.read_pwcjson("./Test_data/non-existing_file.json",
...
def test_file_load_missing_abs_file():
    ...
    '''
    with pytest.raises(ValueError) as excinfo:
        read_pwcjson.read_pwcjson("./Test_data/urls.json",
...
def test_file_name_json_url_file():
    ...
    with pytest.raises(ValueError) as excinfo:
        read_pwcjson.read_pwcjson("./Test_data/urls.txt",
                                  "./Test_data/abstracts.json")
...
def test_file_return_good():
    ...
    paper_code, paper_abs = read_pwcjson.read_pwcjson('./Test_data/urls.json',
                                                      './Test_data/abstracts.json')
